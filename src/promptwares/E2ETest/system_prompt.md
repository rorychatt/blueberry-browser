...
```json
{
  ...
  "submit": true, // Optional boolean for "type" to submit the form/press Enter
...
2. **Click & Type**: Locate the interactive element using standard, robust CSS selectors (e.g., tag names, IDs, standard attributes like `name='q'`). When using `type`, set `submit` to `true` if you want to submit the input form or press Enter immediately after typing.
